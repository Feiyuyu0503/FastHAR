...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        fps_idx = farthest_point_sample(xyz, S).long()  # [B, npoint]
...
class PreExtraction(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(-1, d, s) # torch.Size([-1,6,32])
        x = self.transfer(x)
...
class PointMLPEncoder(nn.Module):
    def __init__(self, args,in_channels=3, embed_dim=64, groups=1, res_expansion=1.0,
